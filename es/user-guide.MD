...
### 4.19. Iniciar aplicaciones automáticamente al iniciar i3
Usando la palabra clave `exec` fuera de una combinación de teclas, puedes configurar qué comandos serán ejecutados por i3 en el inicio inicial. Los comandos `exec` no se ejecutarán al reiniciar i3; si necesitas que un comando se ejecute también al reiniciar i3, debes usar la palabra clave `exec_always`. Estos comandos se ejecutarán en orden.

TODO

Consulta [command_chaining] para obtener detalles sobre el significado especial de `;` (punto y coma) y `,` (coma): encadenan comandos en i3, por lo que necesitas usar cadenas entre comillas (como se muestra en [exec_quoting]) si aparecen en tu comando.
...
### 4.20. Colocar automáticamente espacios de trabajo en pantallas específicas

Si asignas clientes a espacios de trabajo, puede ser útil colocar los espacios de trabajo en pantallas específicas. Además, la asignación de espacios de trabajo a pantallas determinará qué espacio de trabajo usa i3 para una nueva pantalla al agregar pantallas o al iniciar (por ejemplo, de forma predeterminada, usará 1 para la primera pantalla, 2 para la segunda pantalla y así sucesivamente).
...
El `output` es el nombre de la salida RandR a la que conectas tu pantalla. En un portátil, podrías tener VGA1 y LVDS1 como nombres de salida. Puedes ver las salidas disponibles ejecutando `xrandr --current`.

Si tu servidor X soporta RandR 1.5 o más reciente, i3 usará objetos de monitor RandR en lugar de objetos de salida. Ejecuta `xrandr --listmonitors` para ver una lista. Por lo general, un objeto de monitor contiene exactamente una salida y tiene el mismo nombre que la salida; pero si ese no es el caso, puedes especificar el nombre del monitor o de la salida en la configuración de i3. Por ejemplo, el Dell UP2414Q usa dos escaladores internamente, por lo que sus nombres de salida podrían ser “DP1” y “DP2”, pero el nombre del monitor es “Dell UP2414Q”.

(Ten en cuenta que incluso si especificas el nombre de una salida que no abarca todo el monitor, i3 seguirá usando toda el área del monitor contenedor en lugar de solo la del output).

Puedes especificar múltiples salidas. Se usará la primera disponible.

Si usas espacios de trabajo con nombre, deben ir entre comillas:
...
### 4.21. Cambiar colores

Puedes cambiar todos los colores que i3 utiliza para dibujar las decoraciones de las ventanas.
...
Donde colorclass puede ser uno de los siguientes:

`client.focused`
Un cliente que actualmente tiene el foco.

`client.focused_inactive`
Un cliente que es el enfocado de su contenedor, pero que no tiene el foco en este momento.

`client.focused_tab_title`
Título del contenedor de pestaña o pila que es el padre del contenedor enfocado, pero que no está directamente enfocado. Por defecto, utiliza focused_inactive si no se especifica y no usa los colores de indicator y child_border.

`client.unfocused`
Un cliente que no es el enfocado de su contenedor.

`client.urgent`
Un cliente que tiene activado su indicador de urgencia.

`client.placeholder`
El color de fondo y el color del texto se utilizan para dibujar el contenido de la ventana de marcador de posición (cuando se restauran diseños). El borde y el indicador se ignoran.

`client.background`
Color de fondo que se usará para pintar el fondo de la ventana del cliente sobre la cual se renderizará el cliente. Solo los clientes que no cubren toda el área de esta ventana exponen el color. Ten en cuenta que esta clase de color solo toma un solo color.

Los colores están en formato hexadecimal HTML (#rrggbb, opcionalmente #rrggbbaa), ve el siguiente ejemplo:

Ejemplos (colores predeterminados):
...
Ten en cuenta que para las decoraciones de la ventana, el color alrededor de la ventana secundaria es el "child_border", y el color "border" son solo las dos líneas delgadas alrededor de la barra de título.

El color del indicador se utiliza para indicar dónde se abrirá una nueva ventana. Para los contenedores divididos horizontalmente, el borde derecho se pintará con el color del indicador; para los contenedores divididos verticalmente, el borde inferior. Esto solo se aplica a ventanas individuales dentro de un contenedor dividido, que de otro modo son indistinguibles de las ventanas individuales fuera de un contenedor dividido.

### 4.22. Comunicación entre procesos

i3 utiliza sockets Unix para proporcionar una interfaz IPC. Esto permite a programas de terceros obtener información de i3, como los espacios de trabajo actuales (para mostrar una barra de espacios de trabajo) y controlar i3.

Por defecto, se creará un socket IPC en `$XDG_RUNTIME_DIR/i3/ipc-socket.%p` si el directorio está disponible, y como alternativa en `/tmp/i3-%u.XXXXXX/ipc-socket.%p`, donde `%u` es tu nombre de usuario de UNIX, `%p` es el PID de i3 y XXXXXX es una cadena de caracteres aleatorios del conjunto de caracteres de nombre de archivo portátil (ver mkdtemp(3)).

Puedes anular la ruta predeterminada a través de la variable de entorno `I3SOCK` o especificando la directiva `ipc-socket`. Sin embargo, esto no se recomienda, ya que i3 hace lo correcto por defecto. Si decides cambiarlo, se recomienda encarecidamente establecerlo en una ubicación en tu directorio personal para que ningún otro usuario pueda crear ese directorio.
...
Luego puedes usar la aplicación i3-msg para ejecutar cualquier comando listado en [list_of_commands].

### 4.23. El foco sigue al ratón

Por defecto, el foco de la ventana sigue los movimientos de tu ratón cuando el ratón cruza los bordes de las ventanas. Sin embargo, si tienes una configuración donde tu ratón suele estar en tu camino (como un touchpad en tu portátil que no deseas desactivar por completo), es posible que desees desactivar el `foco sigue al ratón (focus follows mouse)` y controlar el foco solo usando tu teclado. El ratón seguirá siendo útil dentro de la ventana actualmente activa (por ejemplo, para hacer clic en enlaces en tu ventana del navegador).
...
### 4.24. Teletransporte del ratón

Por defecto, al cambiar el foco a una ventana en una salida diferente (por ejemplo, enfocando una ventana en el espacio de trabajo 3 en la salida VGA-1, viniendo del espacio de trabajo 2 en la salida LVDS-1), el cursor del ratón se mueve al centro de esa ventana.

Con la opción `mouse_warping`, puedes controlar cuándo debe teletransportarse el cursor del ratón. `none` desactiva el teletransporte por completo, mientras que `output` es el comportamiento predeterminado descrito anteriormente.
...
### 4.25. Ventanas emergentes durante el modo de pantalla completa

Cuando estás en modo de pantalla completa, algunas aplicaciones aún abren ventanas emergentes (toma Xpdf como ejemplo). Esto se debe a que estas aplicaciones pueden no ser conscientes de que están en modo de pantalla completa (no verifican la pista correspondiente). Hay tres cosas que se pueden hacer en esta situación:

1. Mostrar la ventana emergente solo si pertenece a la aplicación en pantalla completa. Este es el valor predeterminado y debería ser un comportamiento razonable para la mayoría de los usuarios.

2. Ignorar la ventana emergente (no mapearla). Esto no te interrumpirá mientras estés en pantalla completa. Sin embargo, algunas aplicaciones pueden reaccionar mal a esto (bloqueo hasta que salgas de la pantalla completa).

3. Salir del modo de pantalla completa.
...
### 4.26. Envolvimiento del foco

Por defecto, cuando estás en un contenedor con varias ventanas o contenedores secundarios, la ventana opuesta se enfocará al intentar mover el foco sobre el borde de un contenedor (y no hay otros contenedores en esa dirección), es decir, el foco se envuelve.

Si lo deseas, puedes desactivar este comportamiento configurando la directiva `focus_wrapping` con el valor `no`.

Cuando está habilitado, el envolvimiento del foco no ocurre por defecto si hay otra ventana o contenedor en la dirección especificada, y el foco se establecerá en esa ventana o contenedor. Este es el comportamiento predeterminado para que puedas navegar a todas tus ventanas sin tener que usar `focus parent`.

Si deseas que el foco `siempre` se envuelva y eres consciente de usar `focus parent` para cambiar a diferentes contenedores, puedes configurar focus_wrapping con el valor `force`.

Para restringir el foco dentro del espacio de trabajo actual, configura `focus_wrapping` con el valor `workspace`. Necesitarás usar `focus parent` hasta que se seleccione un espacio de trabajo para cambiar a un espacio de trabajo diferente usando los comandos de foco (el comando `workspace` seguirá funcionando como se espera).
...
### 4.27. Forzar Xinerama

Como se explica en detalle en [https://i3wm.org/docs/multi-monitor.html](https://i3wm.org/docs/multi-monitor.html), algunos controladores de video X11 (especialmente el controlador binario de nVidia) solo proporcionan soporte para Xinerama en lugar de RandR. En tal situación, i3 debe ser configurado explícitamente para usar la API inferior de Xinerama y, por lo tanto, no proporcionar soporte para reconfigurar tus pantallas sobre la marcha (se leen solo una vez al iniciar y eso es todo).

Para las personas que no pueden modificar su `~/.xsession` para agregar el parámetro de línea de comandos `--force-xinerama`, se proporciona una opción de configuración:
...
También ten en cuenta que los nombres de tus salidas no son descriptivos (como `HDMI1`) cuando usas Xinerama, en su lugar se enumeran comenzando desde 0: `xinerama-0`, `xinerama-1`, ...

### 4.28. Cambio automático de ida y vuelta al cambiar al espacio de trabajo actual

TODO

Esta directiva de configuración habilita el cambio automático de `back_and_forth` del espacio de trabajo (ver [back_and_forth]) al cambiar al espacio de trabajo que está actualmente enfocado.

Por ejemplo: Supongamos que estás en el espacio de trabajo "1: www" y cambias a "2: IM" usando mod+2 porque alguien te envió un mensaje. No necesitas recordar de dónde vienes ahora, solo puedes presionar $mod+2 nuevamente para volver a "1: www".
...
### 4.29. Retrasar el restablecimiento del indicador de urgencia al cambiar de espacio de trabajo

Si una aplicación en otro espacio de trabajo establece un indicador de urgencia, cambiar a este espacio de trabajo puede llevar al enfoque inmediato de la aplicación, lo que también significa que el color de la decoración de la ventana se restablecería inmediatamente a `client.focused`. Esto podría dificultar innecesariamente determinar qué ventana originó el evento.

Para evitar esto, puedes decirle a i3 que retrase el restablecimiento del estado de urgencia durante un cierto tiempo usando la directiva `force_display_urgency_hint`. Establecer el valor en 0 desactiva esta función.

El valor predeterminado es 500ms.
...
Si una ventana se activa, por ejemplo, a través de google-chrome www.google.com, puede solicitar tomar el foco. Dado que esto podría no ser preferible, se pueden configurar diferentes reacciones.

Ten en cuenta que esto podría no afectar a las ventanas que se están abriendo. Para evitar que las ventanas nuevas se enfoquen, consulta [no_focus].
...
`smart`
Este es el comportamiento predeterminado. Si la ventana que solicita el foco está en un espacio de trabajo activo, recibirá el foco. De lo contrario, se establecerá el indicador de urgencia.

`urgent`
La ventana siempre se marcará como urgente, pero no se robará el foco.

`focus`
La ventana siempre se enfocará y no se marcará como urgente.

`none`
La ventana no será enfocada ni marcada como urgente.

### 4.31. Dibujar marcas en la decoración de la ventana

TODO

Si está activado, las marcas (consulta [vim_like_marks]) en las ventanas se dibujan en su decoración. Sin embargo, cualquier marca que comience con un guion bajo en su nombre (`_`) no se dibujará, incluso si esta opción está activada.

El valor predeterminado para esta opción es `yes`.
